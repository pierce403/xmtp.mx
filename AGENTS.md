...
- Wins: Outbound SMTP compose uses the existing relay's public production inbox ID through `NEXT_PUBLIC_XMTP_RELAY_INBOX_ID` and sends the established `email.send.v1` JSON envelope over XMTP.
